...
.accordion-content{overflow:hidden;max-height:0;transition:max-height .4s ease}.accordion-content.open{max-height:99999px}
...
.ep-desc{font-size:12px;opacity:.7;margin-top:4px;line-height:1.4}.ep-desc a{color:var(--violet);text-decoration:underline;font-weight:600}
...
.ep-result{margin-top:8px;display:none}.ep-result.show{display:block}
...
.token-bar button{font-family:var(--display);font-weight:700;font-size:11px;padding:8px 16px;border:2px solid var(--ink);box-shadow:2px 2px 0 var(--ink);cursor:pointer;transition:all .12s;white-space:nowrap;color:var(--ink)}
...
.ft-cols a{display:block;font-size:14px;font-weight:500;margin-bottom:8px;opacity:.85;transition:opacity .2s,transform .2s;color:var(--bg)}
.ft-cols a:hover{opacity:1;transform:translateX(3px)}
.ft-social{display:flex;gap:12px;flex-wrap:wrap}
.ft-social a{display:inline-flex;width:48px;height:48px;align-items:center;justify-content:center;text-decoration:none;border:2px solid var(--bg);background-size:100% 200%;background-position:0% 5%;transition:background-position .5s,color .5s}
...
.ft-note{grid-column:1/-1;font-family:var(--mono);font-size:12px;opacity:.5;border-top:1.5px solid rgba(255,255,255,.2);padding-top:18px;margin-top:20px;color:var(--bg)}
...
<body>
<div class="marquee"><div class="marquee-track"><span>API ABSENSI SEKOLAH v${version}</span><span>|</span><span>QR CODE</span><span>|</span><span>WHATSAPP BOT</span><span>|</span><span>GOOGLE SHEETS</span><span>|</span><span>MONITORING</span><span>|</span><span>EXPORT EXCEL</span><span>|</span><span>IZIN & SAKIT</span><span>|</span><span>PENGUMUMAN</span><span>|</span><span>CHANNEL BERITA</span><span>|</span><span>API ABSENSI SEKOLAH v${version}</span><span>|</span><span>QR CODE</span><span>|</span><span>WHATSAPP BOT</span><span>|</span><span>GOOGLE SHEETS</span><span>|</span><span>MONITORING</span><span>|</span><span>EXPORT EXCEL</span><span>|</span><span>IZIN & SAKIT</span><span>|</span><span>PENGUMUMAN</span><span>|</span><span>CHANNEL BERITA</span></div></div>
<header class="site-header">
<button class="sidebar-trigger" id="sidebarTrigger">&#9776;</button>
<nav class="breadcrumb" id="breadcrumbNav"><span class="current">API Docs</span></nav>
...
<div class="theme-toggle" id="themeToggle"><button data-theme="dark" class="active">D</button><button data-theme="light">L</button></div>
</div>
</header>
<div class="app-layout">
<aside class="app-sidebar" id="appSidebar"><div class="sidebar-inner"><div class="sidebar-header"><span>NAVIGASI</span></div><div class="sidebar-search"><input type="text" id="sidebarSearch" placeholder="Cari endpoint..."></div><nav class="sidebar-nav" id="sidebarNav"></nav><div class="no-result" id="noResult">Tidak ditemukan</div></div></aside>
<main class="sidebar-inset" id="sidebarInset">
<div class="hero-section"><div class="hero-card"><div class="hero-img"><span class="hero-tag">v${version}</span><img src="${THUMBNAIL_URL}" alt="API Thumbnail" loading="lazy" onerror="this.style.display='none'"><div class="hero-img-fallback">{API}</div><div class="hero-img-sub">REST API v2.0</div></div><div class="hero-body"><h2>API Absensi Sekolah</h2><p>Backend API untuk sistem absensi berbasis <b>QR Code</b>, <b>WhatsApp Bot</b>, dan <b>Google Sheets</b>.</p><div class="hero-stats"><div class="hero-stat"><b>${endpointCount}</b>Endpoints</div><div class="hero-stat"><b>JWT</b>Auth</div></div><div class="hero-cta"><a href="${FRONTEND_URL}" class="cta-primary frontend-link">Buka Frontend</a><a href="/api/docs/json" target="_blank" class="cta-secondary">JSON Spec</a></div></div></div></div>
<div class="token-bar"><label>TOKEN:</label><input type="text" id="globalToken" placeholder="Paste JWT token..."><button class="primary" id="saveTokenBtn">SIMPAN</button><button id="clearTokenBtn" style="background:var(--pink);color:var(--ink)">HAPUS</button></div>
<div class="scroll-area" id="scrollArea"><div id="accordionContainer"></div></div>
</main>
</div>
<footer class="site-footer"><div class="ft-big">ABSENSI<br>API</div><div class="ft-cols"><div><h4>NAVIGASI</h4><a href="${FRONTEND_URL}" target="_blank">Frontend</a><a href="/api/health" target="_blank">Health Check</a></div><div><h4>SOSIAL MEDIA</h4><div class="ft-social"><a href="${SOCIALS.instagram}" target="_blank" title="Instagram"><svg viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/><circle cx="17.5" cy="6.5" r="1.5"/></svg></a><a href="${SOCIALS.youtube}" target="_blank" title="YouTube"><svg viewBox="0 0 24 24"><polygon points="10,8 16,12 10,16"/><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816z"/></svg></a><a href="${SOCIALS.tiktok}" target="_blank" title="TikTok"><svg viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg></a><a href="${SOCIALS.github}" target="_blank" title="GitHub"><svg viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg></a></div></div></div><div class="ft-note">&copy; ${year} Absensi Sekolah API. <a href="${FRONTEND_URL}" target="_blank">${FRONTEND_URL}</a></div></footer>
<script>
(function(){
var S = ${specJson};
var BASE = "${BASE_URL}";
...
document.getElementById("saveTokenBtn").addEventListener("click",function(){token=document.getElementById("globalToken").value.trim();localStorage.setItem("api-token",token);alert("Token disimpan!")});
document.getElementById("clearTokenBtn").addEventListener("click",function(){token="";document.getElementById("globalToken").value="";localStorage.removeItem("api-token")});
var paths = S.paths;
var grouped = {};
...
var allLinks=[];var gi=0;
Object.entries(grouped).forEach(function(e){var tag=e[0],eps=e[1];var idx=gi;gi++;
var link=document.createElement("a");link.href="#";link.textContent=tag;link.setAttribute("data-tag",tag.toLowerCase());
link.addEventListener("click",function(ev){ev.preventDefault();var t=document.getElementById("group-"+idx);if(t){document.getElementById("scrollArea").scrollTo({top:t.offsetTop-20,behavior:"smooth"});var tr=t.querySelector(".accordion-trigger");if(tr&&!tr.classList.contains("open"))toggleAccordion(tr);updateBreadcrumb(tag)}document.querySelectorAll(".sidebar-nav a").forEach(function(a){a.classList.remove("active")});link.classList.add("active")});
sidebarNav.appendChild(link);allLinks.push({link:link,tag:tag.toLowerCase()});
var item=document.createElement("div");item.className="accordion-item";item.id="group-"+idx;var count=eps.length;
var trigger=document.createElement("button");trigger.className="accordion-trigger";
trigger.innerHTML="<span>"+tag+"</span><span style='display:flex;align-items:center;gap:8px'><span class='badge'>"+count+"</span><svg class='chevron' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><polyline points='6 9 12 15 18 9'></polyline></svg></span>";
trigger.addEventListener("click",function(){toggleAccordion(this)});
var content=document.createElement("div");content.className="accordion-content";var inner=document.createElement("div");inner.className="accordion-content-inner";
...
var sHTML="";var sObj=null;
if(ep.requestBody){var ex=ep.requestBody&&ep.requestBody.content&&ep.requestBody.content["application/json"]&&ep.requestBody.content["application/json"].example;if(ex){sObj=ex;sHTML="<div class='ep-sample'><strong>Request:</strong>\n"+JSON.stringify(ex,null,2)+"</div>"}}
var pParams="";if(ep.path.indexOf("{")>-1){var m=ep.path.match(/\{[^}]+\}/g);if(m){pParams="<div class='ep-desc'><strong>Params:</strong> "+m.join(", ").replace(/[{}]/g,"")+"</div>"}}
var aBadge="";if(ep.security&&ep.security.length>0){aBadge="<span style='font-family:var(--mono);font-size:9px;padding:2px 6px;border:1.5px solid var(--ink);background:var(--yellow);margin-left:6px'>AUTH</span>"}else{aBadge="<span style='font-family:var(--mono);font-size:9px;padding:2px 6px;border:1.5px solid var(--ink);background:var(--teal);margin-left:6px'>PUBLIC</span>"}
var desc=(ep.summary||"")+(ep.description?" - "+ep.description.replace(/\*\*/g,"").replace(/\n/g," "):"");
card.innerHTML="<div class='ep-head'><div class='ep-head-left'><span class='ep-method "+mc+"'>"+ep.method+"</span><span class='ep-path'>"+ep.path+"</span>"+aBadge+"</div><button class='ep-try-btn' id='try-"+epId+"'>TRY IT</button></div><div class='ep-desc'>"+desc+"</div>"+pParams+sHTML+"<div class='ep-result' id='result-"+epId+"'><div class='ep-result-header'><span class='ep-result-status' id='status-"+epId+"'></span><span id='time-"+epId+"' style='opacity:.6'></span></div><pre class='ep-result-body' id='body-"+epId+"'></pre></div>";
inner.appendChild(card);
setTimeout(function(){var btn=document.getElementById("try-"+epId);if(!btn)return;btn.addEventListener("click",function(){executeEndpoint(epId,ep.method,ep.path,sObj,!!(ep.security&&ep.security.length>0))})},10)});
content.appendChild(inner);item.appendChild(trigger);item.appendChild(content);accordionContainer.appendChild(item)});

function executeEndpoint(epId,method,path,sObj,needsAuth){var rd=document.getElementById("result-"+epId);var se=document.getElementById("status-"+epId);var be=document.getElementById("body-"+epId);var te=document.getElementById("time-"+epId);var btn=document.getElementById("try-"+epId);rd.classList.add("show");se.textContent="...";se.className="ep-result-status";be.textContent="Loading...";te.textContent="";btn.classList.add("loading");btn.textContent="...";var url=BASE+path;var pp=path.match(/\{[^}]+\}/g);if(pp){pp.forEach(function(p){var pn=p.replace(/[{}]/g,"");var val=prompt("Masukkan "+pn+":","");if(val)url=url.replace(p,val);else url=url.replace(p,pn)})}var st=Date.now();try{var hd={"Content-Type":"application/json"};if(needsAuth&&token)hd["Authorization"]="Bearer "+token;var fo={method:method,headers:hd};if(method!=="GET"&&method!=="HEAD"&&sObj)fo.body=JSON.stringify(sObj);fetch(url,fo).then(function(r){return r.text().then(function(t){var f;try{f=JSON.stringify(JSON.parse(t),null,2)}catch(e){f=t}var ok=r.ok?"ok":"err";se.textContent=r.status+" "+(r.ok?"OK":"ERR");se.className="ep-result-status "+(r.ok?"success":"error");be.textContent=f;te.textContent=(Date.now()-st)+"ms";btn.classList.remove("loading");btn.textContent="TRY IT"})}).catch(function(e){se.textContent="ERR";se.className="ep-result-status error";be.textContent=e.message;te.textContent=(Date.now()-st)+"ms";btn.classList.remove("loading");btn.textContent="TRY IT"})}catch(e){se.textContent="ERR";se.className="ep-result-status error";be.textContent=e.message;btn.classList.remove("loading");btn.textContent="TRY IT"}}

var sidebar=document.getElementById("appSidebar");var isOpen=true;
document.getElementById("sidebarTrigger").addEventListener("click",function(){isOpen=!isOpen;sidebar.classList.toggle("collapsed",!isOpen);localStorage.setItem("sidebar-open",isOpen?"1":"0")});
if(localStorage.getItem("sidebar-open")==="0"){isOpen=false;sidebar.classList.add("collapsed")}
...
var si=document.getElementById("sidebarSearch");var nr=document.getElementById("noResult");
si.addEventListener("input",function(){var q=this.value.toLowerCase().trim();var f=0;allLinks.forEach(function(item){if(q===""||item.tag.includes(q)){item.link.classList.remove("hidden");f++}else{item.link.classList.add("hidden")}});if(q!==""&&f===0)nr.classList.add("show");else nr.classList.remove("show")});
function toggleAccordion(trigger){var c=trigger.nextElementSibling;var io=c.classList.contains("open");var p=trigger.parentNode.parentNode;if(p){p.querySelectorAll(".accordion-content").forEach(function(x){x.classList.remove("open")});p.querySelectorAll(".accordion-trigger").forEach(function(x){x.classList.remove("open")})}if(!io){c.classList.add("open");trigger.classList.add("open")}}
...
var fl=document.querySelectorAll("a.frontend-link, a[href='"+FE+"']");fl.forEach(function(a){a.addEventListener("click",function(e){if(a.getAttribute("target")==="_blank")return;e.preventDefault();window.open(a.href,"_blank")})});
})();
</script>
</body>
</html>`;
