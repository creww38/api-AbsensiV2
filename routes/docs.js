...
const openApiSpec = {
  openapi: '3.1.0',
  info: { title: 'API Absensi Sekolah', version: packageInfo.version || '2.0.0' },
...
<style>
:root{--bg:#ece7ff;--ink:#16101f;--paper:#ffffff;--violet:#8b5cf6;--pink:#ff4d97;--teal:#14d6c4;--yellow:#ffd633;--lime:#b4e82a;--orange:#ff7a3d;--bd:3px solid var(--ink);--sh:6px 6px 0 var(--ink);--sh-lg:9px 9px 0 var(--ink);--sh-sm:4px 4px 0 var(--ink);--display:"Syne",sans-serif;--sans:"Space Grotesk",system-ui,sans-serif;--mono:"DM Mono",monospace}
...
body{background:var(--bg);color:var(--ink);font-family:var(--sans);font-size:15px;line-height:1.5;-webkit-font-smoothing:antialiased;background-image:radial-gradient(var(--ink) 1.2px,transparent 1.2px);background-size:26px 26px;background-position:-13px -13px;transition:background .3s,color .3s}
...
/* HERO */
.hero{max-width:1240px;margin:0 auto;padding:40px 26px 30px;display:grid;grid-template-columns:1fr 280px;gap:26px;align-items:stretch}
.hero-main{position:relative}
.hero-sticker{position:absolute;top:-6px;right:20px;background:var(--lime);border:var(--bd);box-shadow:var(--sh-sm);font-family:var(--display);font-weight:800;font-size:16px;line-height:.95;text-align:center;padding:12px 14px;transform:rotate(8deg);z-index:2}
.hero-title{font-family:var(--display);font-weight:800;font-size:clamp(3.2rem,11vw,8rem);line-height:.86;letter-spacing:-.03em}
.hero-title .outline{color:transparent;-webkit-text-stroke:3px var(--ink)}
.hero-sub{margin-top:22px;font-size:17px;max-width:460px;font-weight:500}
.hero-sub b{background:var(--yellow);padding:0 4px;box-decoration-break:clone}
.hero-card{background:var(--violet);border:var(--bd);box-shadow:var(--sh);color:var(--paper);padding:18px;display:flex;flex-direction:column;gap:0}
.hc-row{display:flex;justify-content:space-between;align-items:center;font-family:var(--mono);font-size:12px;padding:7px 0;border-bottom:1.5px solid rgba(255,255,255,.25)}
.hc-row b{font-weight:500}.hc-big{font-family:var(--display);font-weight:800;font-size:2.6rem;margin-top:auto;padding-top:16px;line-height:1}.hc-cat{font-size:13px;opacity:.85;margin-top:4px}

/* SIDEBAR OVERLAY */
.sidebar-overlay{position:fixed;inset:0;background:rgba(22,16,31,.5);opacity:0;visibility:hidden;transition:opacity .3s,visibility .3s;z-index:90}
.sidebar-overlay.open{opacity:1;visibility:visible}
.sidebar-drawer{position:fixed;top:0;left:0;height:100%;width:320px;max-width:90vw;background:var(--bg);border-right:var(--bd);z-index:100;display:flex;flex-direction:column;transform:translateX(-100%);transition:transform .35s cubic-bezier(.16,1,.3,1)}
.sidebar-drawer.open{transform:translateX(0)}
.sidebar-header{display:flex;justify-content:space-between;align-items:center;padding:18px 20px;border-bottom:var(--bd);background:var(--violet);color:var(--paper)}
.sidebar-header h3{font-family:var(--display);font-weight:800;font-size:1.2rem}
.sidebar-close{background:var(--paper);color:var(--ink);border:var(--bd);box-shadow:var(--sh-sm);width:36px;height:36px;font-size:16px;transition:all .12s}
.sidebar-close:hover{transform:translate(-2px,-2px);box-shadow:var(--sh)}
.sidebar-search{padding:12px 16px;border-bottom:var(--bd)}
...
/* TOKEN BAR */
.token-bar{max-width:1240px;margin:0 auto 20px;padding:0 26px}
.token-bar-inner{background:var(--paper);border:var(--bd);box-shadow:var(--sh-sm);padding:12px 16px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}
...
/* GRID */
.grid{max-width:1240px;margin:0 auto;padding:10px 26px 60px;display:grid;grid-template-columns:repeat(3,1fr);gap:22px}
...
.item-head h3{font-family:var(--display);font-weight:800;font-size:1.3rem;line-height:1;letter-spacing:-.02em}
.price-tag{font-family:var(--display);font-weight:800;font-size:1rem;background:var(--ink);color:var(--paper);padding:5px 10px;transform:rotate(3deg);white-space:nowrap;box-shadow:var(--sh-sm)}
.description{font-size:12px;font-weight:500;line-height:1.5;overflow:hidden}
.ep-list{display:flex;flex-direction:column;gap:6px;margin-top:4px}
.ep-row{display:flex;align-items:center;gap:6px;font-family:var(--mono);font-size:10px}
.ep-method{font-weight:700;padding:2px 6px;border:2px solid var(--ink);box-shadow:2px 2px 0 var(--ink);font-size:9px}
...
.ep-path{font-weight:600;font-size:10px;word-break:break-all}
.ep-badge{font-size:8px;padding:1px 4px;border:1.5px solid var(--ink)}
...
.try-btn{width:100%;background:var(--ink);color:var(--paper);border:var(--bd);box-shadow:var(--sh-sm);padding:10px;font-family:var(--display);font-weight:800;font-size:12px;letter-spacing:.04em;transition:all .12s}
.try-btn:hover{transform:translate(-2px,-2px);box-shadow:var(--sh);background:var(--paper);color:var(--ink)}
.expand-btn{width:100%;background:var(--lime);color:var(--ink);border:var(--bd);box-shadow:var(--sh-sm);padding:10px;font-family:var(--display);font-weight:800;font-size:12px;letter-spacing:.04em;transition:all .12s}
.expand-btn:hover{transform:translate(-2px,-2px);box-shadow:var(--sh)}
.result-box{margin-top:8px;display:none;border:var(--bd);background:var(--paper);padding:10px;font-family:var(--mono);font-size:10px;max-height:200px;overflow-y:auto;white-space:pre-wrap}
...
.ft-social a{display:grid;place-items:center;width:40px;height:40px;border:2px solid var(--bg);transition:all .12s;padding:6px}
...
.ft-note{grid-column:1/-1;font-family:var(--mono);font-size:12px;opacity:.5;border-top:1.5px solid rgba(255,255,255,.2);padding-top:18px;margin-top:10px}
.ft-note a{text-decoration:underline;opacity:.8}.ft-note a:hover{opacity:1}

@media(max-width:1080px){.grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:860px){.hero{grid-template-columns:1fr}.site-footer{grid-template-columns:1fr;gap:28px}}
@media(max-width:560px){.marquee-track{font-size:11px}.grid{grid-template-columns:1fr;padding:10px 18px 48px}.site-header{padding:12px 18px}.hero{padding:28px 18px 20px}.ft-cols{flex-wrap:wrap;gap:28px}}
</style>
</head>
<body>
...
<button class="cart-toggle" id="sidebarTrigger" style="display:flex;align-items:center;gap:10px;background:var(--pink);color:var(--ink);border:var(--bd);box-shadow:var(--sh-sm);padding:10px 16px;font-family:var(--display);font-weight:800;font-size:14px;letter-spacing:.04em;transition:all .12s">
<span>NAV</span><span class="cart-badge" id="endpointCount">${epCount}</span>
</button>
</header>
...
<p class="hero-sub">Backend API untuk sistem absensi sekolah berbasis <b>QR Code</b>, <b>WhatsApp Bot</b>, dan <b>Google Sheets</b>.</p>
</div>
<div class="hero-card">
...
<div class="hc-row"><span>DATABASE</span><b>GOOGLE SHEETS</b></div>
<div class="hc-big">REST API</div>
<div class="hc-cat"><a href="${FRONTEND_URL}" target="_blank" style="color:var(--paper);text-decoration:underline">Buka Frontend →</a></div>
</div>
</section>
...
(function(){
  ...
  Object.entries(S.paths).forEach(function(e){
    ...
    Object.entries(m).forEach(function(x){
      ...
      grouped[t].push({path:p,method:mt.toUpperCase(),summary:d.summary||'',security:d.security});
    });
  });
  ...
  Object.entries(grouped).forEach(function(e){
    ...
    slink.addEventListener('click',function(ev){ev.preventDefault();
      var el=document.getElementById('cat-'+tag.replace(/[^a-z0-9]/gi,''));
    ...
    card.className='item';card.id='cat-'+tag.replace(/[^a-z0-9]/gi,'');
    ...
    card.innerHTML = '<div class="item-body">'+
      ...
      '<div class="item-options">'+
        '<button class="try-btn test-all-btn">TEST ALL</button>'+
    ...
    card.querySelector('.expand-card-btn').addEventListener('click',function(){
      ...
      txt += 'Gunakan TRY IT untuk testing. Token JWT dibutuhkan untuk endpoint AUTH.\n\n'+
        'Contoh login:\nPOST /api/auth/login\nBody: {"username":"admin","password":"admin123"}\n\n'+
        'Copy token dari response, paste di TOKEN bar, lalu test endpoint lain.';
      box.textContent = txt;
    ...
    card.querySelector('.test-all-btn').addEventListener('click',function(){
      ...
      fetch(url,{method:first.method,headers:hd})
  ...
  document.querySelectorAll('#themeToggle button').forEach(function(b){
    ...
    b.addEventListener('click',function(){
      var t=b.dataset.theme;document.documentElement.className=t;
      localStorage.setItem('api-docs-theme',t);
      document.querySelectorAll('#themeToggle button').forEach(function(x){x.classList.toggle('active',x.dataset.theme===t)});
    });
  });
  
  // LOADER
  document.querySelectorAll('a[href="'+FE+'"]').forEach(function(a){
