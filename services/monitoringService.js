  ...
  try {
    ...
    for (let i = 1; i < absensiData.length; i++) {
      ...
      if (rowDateStr === todayStr) {
        absensiMap[nisn] = { jamDatang: absensiData[i][4] || '-', jamPulang: absensiData[i][5] || '-', keterangan: absensiData[i][6] || '-', status: absensiData[i][7] || 'Belum Absen' };
      }
    }
    const result = [];
    for (let i = 1; i < siswaData.length; i++) {
      ...
      result.push({ nama: siswaData[i][0], nisn, kelas, jamDatang: info.jamDatang, jamPulang: info.jamPulang, keterangan: info.keterangan, status: info.status });
    }
...
async function updateStatus(token, nisn, nama, kelas, newStatus) {
  try {
    ...
    if (found) {
      ...
      await appendToSheet(process.env.SHEET_ABSENSI || 'absensi', [new Date().toISOString(), nisn, nama, kelas, '-', '-', '-', newStatus]);
    }
